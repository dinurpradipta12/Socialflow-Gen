...
const Team: React.FC<TeamProps> = ({ primaryColor, currentUser, workspace, onUpdateWorkspace, addSystemNotification, allUsers, setUsers, setWorkspace, onJoinAnotherWorkspace }) => {
  ...
  const handleRemoveMember = (userId: string) => {
    if (userId === workspace.ownerId) { alert("Owner tidak bisa dihapus."); return; }
  ...
  const togglePermission = (key: keyof Permissions) => {
    ...
    setEditingUser({ ...editingUser, permissions: { ...editingUser.permissions, [key]: !editingUser.permissions[key] } });
  };

  const addKpi = () => {
    if (!editingUser || !newKpi.trim()) return;
    setEditingUser({ ...editingUser, kpi: [...editingUser.kpi, newKpi.trim()] });
    setNewKpi('');
  };

  const removeKpi = (index: number) => {
    if (!editingUser) return;
    setEditingUser({ ...editingUser, kpi: editingUser.kpi.filter((_, i) => i !== index) });
  };

  const copyInviteCode = () => {
      navigator.clipboard.writeText(workspace.inviteCode);
      alert(`Kode Workspace disalin: ${workspace.inviteCode}`);
  };
  ...
  return (
    <div className="space-y-8 animate-slide pb-20">
      ...
      {/* MEMBER DETAIL VIEW (BOTTOM MOBILE, CENTER DESKTOP, LIGHT BACKDROP) */}
      {viewingUser && (
          <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center p-0 md:p-6 bg-white/30 backdrop-blur-md">
              <div className="absolute inset-0" onClick={() => setViewingUser(null)}></div>
              <div className="relative bg-white w-full max-w-4xl max-h-[85vh] rounded-t-[3rem] md:rounded-[3rem] shadow-2xl overflow-hidden animate-slide flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-gray-50/50">
                      ...
                      <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm mb-12 md:mb-0">
      ...
      {/* EDIT AUTHORITY (BOTTOM MOBILE, CENTER DESKTOP, LIGHT BACKDROP) */}
      {editingUser && (
        <div className="fixed inset-0 z-[220] flex items-end md:items-center justify-center p-0 md:p-6 bg-white/30 backdrop-blur-md">
          <div className="absolute inset-0" onClick={() => setEditingUser(null)}></div>
          <div className="relative bg-white w-full max-w-2xl max-h-[85vh] rounded-t-[3rem] md:rounded-[3rem] shadow-2xl overflow-hidden animate-slide flex flex-col">
             ...
             <div className="p-8 space-y-10 overflow-y-auto custom-scrollbar flex-1">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {['admin', 'editor', 'viewer', 'superuser'].map(r => (
                      <button key={r} onClick={() => setEditingUser({...editingUser, role: r as any})} className={`py-3 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all ${editingUser.role === r ? 'bg-blue-100 border-blue-200 text-blue-600' : 'bg-gray-50 border-gray-100 text-gray-400'}`}>{r}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {Object.keys(editingUser.permissions).map(key => (
                      <button key={key} onClick={() => togglePermission(key as keyof Permissions)} className={`p-4 rounded-2xl border text-left flex flex-col transition-all ${editingUser.permissions[key as keyof Permissions] ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
                ...
                <div className="space-y-4 pb-12">
                  ...
                  <div className="flex gap-2">
                    <input value={newKpi} onChange={e => setNewKpi(e.target.value)} placeholder="Add New KPI..." className="flex-1 px-5 py-3 bg-gray-50 border border-gray-100 rounded-xl outline-none text-xs font-bold" />
             ...
             <div className="p-8 bg-gray-50/50 flex gap-4 shrink-0 border-t">
                ...
                <button onClick={() => handleUpdateUser(editingUser)} className="flex-1 py-4 bg-blue-500 text-white font-black text-[10px] uppercase rounded-2xl shadow-lg active:scale-95 transition-all">Simpan Authority</button>
             </div>
          </div>
        </div>
      )}
      ...
      {viewMode === 'grid' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {workspace.members.map(u => (
                  <div key={u.id} onClick={() => setViewingUser(u)} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden">
                      ...
                      <div className="flex gap-2 border-t border-gray-50 pt-4">
                          <div className="flex-1 text-center p-2 rounded-2xl bg-gray-50"><p className="text-[10px] font-black text-gray-400 uppercase">KPIs</p><p className="text-xl font-black text-gray-900">{u.kpi.length}</p></div>
                          <div className="flex-1 text-center p-2 rounded-2xl bg-gray-50"><p className="text-[10px] font-black text-gray-400 uppercase">Score</p><p className="text-xl font-black text-emerald-500">{u.performanceScore}</p></div>
                      </div>
                      {isOwner && u.id !== workspace.ownerId && (
                          <button onClick={(e) => { e.stopPropagation(); handleRemoveMember(u.id); }} className="absolute top-4 right-4 p-2 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={16}/></button>
                      )}
                  </div>
              ))}
          </div>
            ...
            <tbody className="divide-y divide-gray-50">
                ...
                <tr key={u.id} onClick={() => setViewingUser(u)} className="hover:bg-gray-50/50 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div><p className="text-sm font-black text-gray-900">{u.name}</p><p className="text-[10px] text-gray-300 font-bold uppercase">{u.email}</p></div>
                    </div>
                    </td>
                    <td className="px-10 py-6"><span className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${u.role === 'superuser' ? 'bg-amber-50 text-amber-500 border border-amber-100' : 'bg-blue-50 text-blue-500 border border-blue-100'}`}>{u.role}</span></td>
                    <td className="px-10 py-6"><span className="text-xs font-bold text-gray-500">{u.jobdesk}</span></td>
                    <td className="px-10 py-6 text-center">
                        {isOwner ? <button onClick={(e) => { e.stopPropagation(); setEditingUser(u); }} className="p-2 text-blue-400 hover:bg-blue-50 rounded-xl transition-all"><Edit3 size={16} /></button> : <button className="p-2 text-gray-300"><UserIcon size={16}/></button>}
                    </td>
                </tr>
                ))}
            </tbody>
