 ...
 * 4. Baris 1 Headers: id | name | email | password | timestamp | status
 * 5. Klik Save (Ikon Disket).
 * 6. Klik Deploy > New Deployment.
 * 7. Pilih Type: "Web App".
 * 8. Execute as: "Me".
 * 9. Who has access: "Anyone" (Sangat Penting!).
 * 10. Klik Deploy & Copy URL Web App yang muncul.
 */
...
const DevPortal: React.FC<DevPortalProps> = ({ 
  ...
  const saveDbConfig = () => {
    if (!tempUrl.includes('script.google.com')) {
      alert("Error: Masukkan URL Deployment Web App Apps Script (berakhir dengan /exec).");
      return;
    }
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
           ...
           <div className="flex gap-4">
             ...
             <div className="px-8 py-4 bg-white/5 border border-white/10 rounded-3xl text-center">
                ...
                <p className="text-3xl font-black">{registrations.filter(r => r.status === 'pending').length}</p>
             </div>
           </div>
        </div>
        ...
        {showScriptInfo && (
          <div className="mt-8 p-8 bg-white/5 rounded-[2.5rem] border border-white/10 animate-slide space-y-4">
             <div className="flex justify-between items-center">
                <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2"><Code size={14}/> Google Apps Script Code</h3>
        ...
        <div className="bg-white rounded-[3rem] border border-gray-100 overflow-hidden shadow-sm">
           <table className="w-full text-left">
              <thead className="bg-gray-50/50">
                 <tr>
                    <th className="px-10 py-6 text-[10px] font-black text-gray-300 uppercase tracking-widest">Profile</th>
                    ...
                    <tr key={r.id} className="hover:bg-gray-50/50 transition-all">
                       ...
                       <td className="px-10 py-6">
                          <div className="flex justify-center gap-3">
                              <button onClick={() => onRegistrationAction(r.id, 'approved')} className="px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase hover:scale-105 active:scale-95 transition-all shadow-md">Terima</button>
                              <button onClick={() => onRegistrationAction(r.id, 'rejected')} className="px-5 py-2.5 bg-rose-500 text-white rounded-xl text-[9px] font-black uppercase hover:scale-105 active:scale-95 transition-all shadow-md">Tolak</button>
                          </div>
                       </td>
                    </tr>
                 ))}
                 {registrations.filter(r => r.status === 'pending').length === 0 && (
                   <tr><td colSpan={2} className="py-20 text-center text-gray-300 font-black uppercase text-[10px] tracking-widest">Tidak ada antrian pendaftaran</td></tr>
                 )}
              </tbody>
           </table>
        </div>
