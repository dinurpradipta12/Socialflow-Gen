...
import { Database, RefreshCw, Zap, CheckCircle, XCircle, ShieldCheck, Mail, Clock, Globe, Download, Cloud, Radio, UserPlus, Phone, Calendar, AlertCircle, Key, RefreshCcw, Send, Trash2, Edit, ChevronRight, UserCheck } from 'lucide-react';
...
const DevPortal: React.FC<DevPortalProps> = ({ 
  ...
  const handleSendDirectEmail = (u: User) => {
    const subject = encodeURIComponent("Kredensial Akses Socialflow Hub - Snaillabs Indonesia");
    const body = encodeURIComponent(
      `Halo ${u.name},\n\n` +
      `Akun Socialflow Anda telah diaktifkan oleh Administrator.\n` +
      `Silakan gunakan kredensial berikut untuk masuk ke dashboard:\n\n` +
      `Portal: socialflow.snaillabs.id\n` +
      `Email: ${u.email}\n` +
      `Security Code: ${u.password || 'Social123'}\n\n` +
      `Masa Aktif: ${u.activationDate} s/d ${u.subscriptionExpiry}\n\n` +
      `Harap simpan kredensial ini dengan aman.\n` +
      `Salam,\nAdministrator Socialflow`
    );
    
    // Direct dispatch using mailto protocol from the specified sender email
    window.location.href = `mailto:${u.email}?cc=cs.socialflow@gmail.com&subject=${subject}&body=${body}`;
  };
              ...
              <table className="w-full text-left min-w-[1100px]">
                 <thead className="bg-gray-50/80 border-b border-gray-100">
                    <tr>
                       ...
                       <th className="px-8 py-6 text-[10px] font-black text-gray-300 uppercase tracking-widest text-center">Dispatcher</th>
                    </tr>
                 </thead>
                 <tbody className="divide-y divide-gray-50">
                    {users.map(u => {
                       ...
                       <tr key={u.id} className={`hover:bg-gray-50/50 transition-all group ${isExpired ? 'opacity-60' : ''}`}>
                          ...
                          <td className="px-8 py-6">
                             <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                <button onClick={() => handleSendDirectEmail(u)} className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-500 rounded-xl hover:bg-blue-500 hover:text-white transition-all shadow-sm" title="Dispatch Credentials (mailto)">
                                   <Send size={14}/>
