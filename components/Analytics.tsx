  ...
  const simulatePull = (platform: 'ig' | 'tiktok') => {
    ...
    setTimeout(() => {
      const mockInsight: PostInsight = {
        ...
        sentiment: 'positive',
        analysis: 'Akun ini menunjukkan tren interaksi organik yang stabil pada jam tayang utama.',
        sourceType: 'manual',
      ...
      setUsername('');
      alert(`Data @${username} berhasil ditarik secara simulasi!`);
    }, 2000);
  };
    ...
    <div className="space-y-8 animate-slide pb-20">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          ...
          <p className="text-gray-400 dark:text-slate-500 font-medium">Monitoring performa lintas platform terintegrasi.</p>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
           ...
           <div className="relative bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3rem] p-10 shadow-2xl border border-gray-100 dark:border-slate-800 animate-slide">
              <div className="text-center space-y-4">
                 ...
                 <h2 className="text-2xl font-black dark:text-white">Social Connect</h2>
                 <p className="text-[11px] text-gray-500 dark:text-slate-400 font-medium">Tarik data publik profil tanpa API secara instan (Headless Sim).</p>
              </div>
              <div className="mt-8 space-y-4">
                 <input value={username} onChange={e => setUsername(e.target.value)} className="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-800 outline-none font-bold text-center dark:text-white focus:border-[var(--primary-color)]" placeholder="@username" />
                 <button onClick={() => simulatePull(showPullModal)} disabled={isPulling !== null || !username} className="w-full py-4 bg-[var(--primary-color)] text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Scrapped Items', val: analyticsData.length, color: 'text-blue-500', icon: Link2 },
          { label: 'Plan Sync', val: analyticsData.filter(d => d.sourceType === 'plan').length, color: 'text-indigo-500', icon: CheckCircle2 },
          { label: 'Direct Pull', val: analyticsData.filter(d => d.sourceType === 'manual').length, color: 'text-amber-500', icon: TrendingUp },
          { label: 'Engagement', val: analyticsData.length ? (analyticsData.reduce((a,b)=>a+b.engagementRate,0)/analyticsData.length).toFixed(1) + '%' : '0%', color: 'text-emerald-500', icon: Activity }
        ].map((s, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col items-center text-center">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
         <div className="p-8 border-b border-gray-50 dark:border-slate-800 flex justify-between items-center">
            <h3 className="text-xl font-black dark:text-white">Insight Database History</h3>
            <button className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase tracking-widest dark:text-slate-300"><FileText size={16}/> Download Report</button>
         </div>
         <div className="overflow-x-auto">
            <table className="w-full text-left">
               <thead className="bg-gray-50/50 dark:bg-slate-800/30">
                  <tr>
                     <th className="px-10 py-5 text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">Platform & URL</th>
                     <th className="px-10 py-5 text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">Metrics</th>
               ...
               <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                     ...
                     <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/20 transition-all">
                        <td className="px-10 py-6">
                           <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-gray-400 dark:text-slate-500 truncate max-w-[200px]">{d.url}</span>
                           </div>
                        </td>
                        <td className="px-10 py-6">
                  ...
                  {analyticsData.length === 0 && (
                     <tr><td colSpan={3} className="py-20 text-center text-gray-300 font-black uppercase text-xs tracking-widest italic">Database kosong. Gunakan fitur Connect atau Analyze Link.</td></tr>
                  )}
               </tbody>
            </table>
         </div>
