    ...
    <div className="space-y-8 animate-slide pb-20">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-4">
           ...
           <button onClick={() => setShowPullModal('tiktok')} className="px-6 py-3 bg-black text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2 active:scale-95 transition-all border border-gray-100">
      ...
      {showPullModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
           ...
           <div className="relative bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl border border-gray-100 animate-slide">
              <div className="text-center space-y-4">
                 ...
                 <p className="text-[11px] text-gray-500 font-medium leading-relaxed">Masukkan username untuk mensimulasikan penarikan data publik akun secara instan.</p>
              </div>
              <div className="mt-8 space-y-4">
                 <input 
                   ...
                   className="w-full px-6 py-4 bg-gray-50 rounded-2xl border border-gray-100 outline-none font-bold text-center text-gray-900 focus:border-[var(--primary-color)] transition-all" 
      ...
      <div className="bg-white rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden ring-4 ring-black/5">
         <div className="p-8 border-b border-gray-50 flex justify-between items-center">
            <h3 className="text-xl font-black text-gray-900 tracking-tight">Insight Database History</h3>
            <button className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-600 transition-all hover:bg-gray-100"><FileText size={16}/> Download Report</button>
         </div>
         <div className="overflow-x-auto">
            <table className="w-full text-left">
               <thead className="bg-gray-50/50">
                  <tr>
                     <th className="px-10 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Post Info</th>
                     <th className="px-10 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Performance Metrics</th>
                     <th className="px-10 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Source</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-gray-50">
                     ...
                     <tr key={d.id} className="hover:bg-gray-50 transition-all">
                        <td className="px-10 py-6">
                           <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-gray-400 truncate max-w-[200px] font-medium">{d.url}</span>
                           </div>
                        </td>
                        <td className="px-10 py-6">
                           <div className="flex gap-4 text-[11px] font-black">
                              ...
                              <span className="text-amber-500 font-bold bg-amber-50 px-2 py-0.5 rounded">ER: {d.engagementRate}%</span>
                           </div>
                        </td>
                        <td className="px-10 py-6 text-right">
                           <span className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-widest ${d.sourceType === 'plan' ? 'bg-indigo-50 text-indigo-500' : 'bg-orange-50 text-orange-500'}`}>
                              {d.sourceType}
                           </span>
                        </td>
                     </tr>
                  ...
                  {analyticsData.length === 0 && (
                     <tr><td colSpan={3} className="py-20 text-center text-gray-300 font-black uppercase text-xs tracking-[0.2em] italic">Database kosong. Mulai scrapping dari tracker atau konten plan.</td></tr>
                  )}
               </tbody>
            </table>
         </div>
