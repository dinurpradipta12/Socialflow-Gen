...
import { Mail, Lock, Loader2, CheckCircle, Bell, X, Shield, ArrowRight } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [primaryColorHex, setPrimaryColorHex] = useState('#1e293b');
  const [accentColorHex, setAccentColorHex] = useState('#3b82f6');
      ...
      <div className="min-h-screen bg-[#FDFDFD] flex items-center justify-center p-6 font-['Inter']">
        <div className="max-w-[400px] w-full bg-white rounded-[2rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.05)] p-10 space-y-8 border border-gray-100 z-10 animate-slide">
          <div className="text-center space-y-4">
            <div className="w-14 h-14 bg-gray-900 rounded-2xl mx-auto flex items-center justify-center text-white text-2xl font-black shadow-2xl">
              SF
            </div>
            <div>
              <h1 className="text-xl font-black text-gray-900">Socialflow Login</h1>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Limited Access Workspace</p>
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-3">
              <div className="relative group">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-gray-900 transition-colors" size={16} />
                <input 
                  type="email" required value={email} onChange={(e) => setEmail(e.target.value)} 
                  className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-100 rounded-xl outline-none font-bold text-gray-700 placeholder-gray-300 focus:bg-white focus:border-gray-900 transition-all text-sm" 
                  placeholder="Email Address" 
                />
              </div>
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-gray-900 transition-colors" size={16} />
                <input 
            ...
            <button 
              ...
              className="w-full py-4 bg-gray-900 text-white font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-black active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? <Loader2 size={16} className="animate-spin" /> : <>Enter Workspace <ArrowRight size={14} /></>}
            </button>
          </form>

          <div className="pt-4 text-center">
            <p className="text-[9px] text-gray-300 font-black uppercase tracking-widest">© 2025 SNAILLABS.ID ALL RIGHTS RESERVED</p>
          </div>
        </div>
      </div>
    );
      ...
      <main className="ml-72 p-12 min-h-screen">
        <header className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{activeWorkspace.name}</h2>
          ...
          <div className="hidden md:flex items-center gap-4">
             <div className="text-right">
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center justify-end gap-1.5">
                  <CheckCircle size={10} /> Active Member
                </p>
                <p className="text-[9px] text-gray-300 font-bold uppercase tracking-wider mt-0.5">ID: {user.id.padStart(4, '0')}</p>
             </div>
          </div>
        </header>
        ...
        {showSystemNotifs && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-gray-900/5 backdrop-blur-sm" onClick={() => setShowSystemNotifs(false)}></div>
            <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden animate-slide">
              ...
              <div className="max-h-[50vh] overflow-y-auto p-6 space-y-3 custom-scrollbar">
                {systemNotifications.length > 0 ? systemNotifications.map(n => (
                  ...
                  <div className="text-center py-10">
                    <p className="text-[10px] text-gray-300 font-black uppercase tracking-widest">Tidak ada pesan baru</p>
                  </div>
                )}
              </div>
            </div>
